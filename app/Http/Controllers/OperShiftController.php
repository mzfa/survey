...
class OperShiftController extends Controller
{
    public function index()
    {
        $data = DB::table('oper_shift')->leftJoin('users', 'oper_shift.created_by', '=', 'users.id')
        ->select([
            'users.username',
            'oper_shift.*',
        ])->whereNull('oper_shift.deleted_at')->get();
        $jenis_pembayaran = DB::table('jenis_pembayaran')->whereNull('jenis_pembayaran.deleted_at')->get();
        return view('oper_shift.index', compact('data','jenis_pembayaran'));
    }
